...
window.AppRequest = function (ip, user, passwd, token) {
  ...
  _this.Login = function (resolve, reject, done) {
    try {
      ...
      if (isNullOrEmpty(tokens.token, true) === true) throw new Error(tokens.message);
      bui.ajax({
        url: 'Login',
        data: {
          uid: _user,
  ...
  _this.GetList = function (params, resolve, reject, done) {
    var config = {
      ...
      onLoad: function onLoad(me, data) {
        try {
          if (data.code === 4002) {
            _this.Login(function (token) {
  ...
  _this.Get = function (params, resolve, reject, done) {
    ...
    bui.ajax($.extend(config, params || {})).done(function (data) {
      try {
        if (isNullOrEmpty(data, true) === true) {
          throw new Error('无效的响应');
        }
...
window.loadData = function (before, done, fail, always) {
  ...
  $.when(getAllAreasTask(), getAllStationsTask()).done(function (v1, v2) {
    setAreas(v1);
    setStations(v2);
    done({
      data1: v1,
...
window.getAllAreasTask = function (params) {
  ...
  try {
    getAllAreas(params || {}, function (data) {
      dtd.resolve(data);
    }, function (err) {
...
window.getAllStationsTask = function (params) {
  ...
  try {
    getAllStations(params || {}, function (data) {
      dtd.resolve(data);
    }, function (err) {
      dtd.reject(err.message);
    });
...
window.getStationTask = function (id, params) {
  ...
  try {
    getStation(id, params || {}, function (data) {
      dtd.resolve(data);
    }, function (err) {
      dtd.reject(err.message);
    });
...
window.getDeviceTask = function (id, params) {
  ...
  try {
    getDevice(id, params || {}, function (data) {
      dtd.resolve(data);
    }, function (err) {
      dtd.reject(err.message);
    });
...
window.getVDevices = function (params, resolve, reject, done) {
  try {
    getAllDevices(params, function (data) {
      var devices = [];
...
window.getVDevicesTask = function (params) {
  ...
  try {
    getVDevices(params || {}, function (data) {
      dtd.resolve(data);
    }, function (err) {
      dtd.reject(err.message);
    });
...
window.getXDevicesTask = function (params) {
  ...
  try {
    getXDevices(params || {}, function (data) {
      dtd.resolve(data);
    }, function (err) {
      dtd.reject(err.message);
    });
...
window.getVDevicesByPid = function (pid, params, resolve, reject, done) {
  try {
    getAllDevicesByPid(pid, params, function (data) {
      var devices = [];
      $.each(data, function (index, item) {
        if (item.Type === '影音设备') {
          devices.push(item);
...
window.getVDevicesByPidTask = function (pid, params) {
  ...
  try {
    getVDevicesByPid(pid, params || {}, function (data) {
      dtd.resolve(data);
    }, function (err) {
      dtd.reject(err.message);
    });
...
window.getXDevicesByPidTask = function (pid, params) {
  ...
  try {
    getXDevicesByPid(pid, params || {}, function (data) {
      dtd.resolve(data);
    }, function (err) {
      dtd.reject(err.message);
    });
...
window.getAllSignalsByPidTask = function (pid, params) {
  ...
  try {
    getAllSignalsByPid(pid, params || {}, function (data) {
      dtd.resolve(data);
    }, function (err) {
      dtd.reject(err.message);
    });
...
window.successdialog = function (message, detail) {
  ...
  bui.confirm({
    ...
    height: 400,
    autoClose: true,
    content: "<div><i class=\"appiconfont appicon-confirmed success\"></i><h3>".concat(message, "</h3>").concat(detail, "</div>"),
    buttons: [{
      name: '我知道了',
...
window.warningdialog = function (message, detail) {
  ...
  bui.confirm({
    ...
    height: 400,
    autoClose: true,
    content: "<div><i class=\"appiconfont appicon-error danger\"></i><h3>".concat(message, "</h3>").concat(detail, "</div>"),
    buttons: [{
      name: '我知道了',
...
window.getAlarmCls1 = function (level) {
  if (level === $level.L1) return 'level1';else if (level === $level.L2) return 'level2';else if (level === $level.L3) return 'level3';else if (level === $level.L4) return 'level4';else return 'level0';
};

window.getAlarmCls2 = function (level) {
  if (level === $level.L1) return 'level1-bg';else if (level === $level.L2) return 'level2-bg';else if (level === $level.L3) return 'level3-bg';else if (level === $level.L4) return 'level4-bg';else return 'level0-bg';
};

window.getAlarmName = function (level) {
  if (level === $level.L1) return '一级告警';else if (level === $level.L2) return '二级告警';else if (level === $level.L3) return '三级告警';else if (level === $level.L4) return '四级告警';else return '正常数据';
};
...
bui.ready(function () {
  ...
  $('#bui-router').on('click', '.btn-back', function (e) {
    bui.back({
      beforeBack: function beforeBack(e) {
