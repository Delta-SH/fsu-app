loader.define(function () {
  ...
  pageview.load = function () {
    ...
    $.when(getStationTask(id), getXDevicesByPidTask(id))
      .done(function (v1, v2) {
        if (isNull(v1) === false) {
          pageview.station = v1;
          _inittab0(v1);
  ...
  function _inittab1(data) {
    ...
    $.each(data, function (index, item) {
      keys.push(item.ID);
      html += `
          <li id="pylon-data-device-${item.ID}" class="bui-btn bui-box" href="pages/main/device.html?id=${item.ID}">
            <div class="bui-icon round success"><i class="appiconfont appicon-server"></i></div>
            <div class="span1">${item.Name} <span class="bui-badges hidden">0</span></div>
