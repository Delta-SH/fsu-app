loader.define(function (require, exports, module) {
  ...
  pageview.init = function () {
    ...
    if (isNull(this.loading) === true) {
      this.loading = bui.loading({
        appendTo: "#pylon-app-curvereport",
        text: "正在加载",
      });
    }
    ...
    if (isNull(this.deviceor) === true) {
      this.deviceor = router.$("#pylon-app-curvereport-deviceer");
    ...
    if (isNull(this.signaler) === true) {
      this.signaler = bui.select({
        trigger: "#pylon-app-curvereport-signal",
        title: "信号列表",
        type: "radio",
        height: "60%",
        autoClose: true,
    ...
    if (isNull(this.submit) === true) {
      this.submit = bui.btn("#pylon-app-curvereport-ok");
  ...
  function _bindselect(devcies) {
    ...
    if (authType === $ssh.Station) {
      ...
      router.$("#pylon-app-curvereport-deviceer .selector").html('<div class="selected-val"></div><div class="selected-val"></div>');
      pageview.deviceer = bui.levelselect({
        data: _options,
        title: "筛选范围",
        trigger: "#pylon-app-curvereport-deviceer .selected-val",
        level: 2,
        field: {
          name: "n",
          value: "d",
      ...
      router.$("#pylon-app-curvereport-deviceer .selector").html('<div class="selected-val"></div><div class="selected-val"></div><div class="selected-val"></div>');
      pageview.deviceer = bui.levelselect({
        data: _options,
        title: "筛选范围",
        trigger: "#pylon-app-curvereport-deviceer .selected-val",
  ...
  function _submit(loading) {
    ...
    loading.stop();
    router.load({
      url: "pages/main/curvedetail.html",
      param: {
        id: _nodes[0].ID,
        name: _nodes[0].Name,
